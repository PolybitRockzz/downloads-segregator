...
img_ex = [".png", ".jpg", ".jpeg", ".gif", ".webp"] # image extensions
        ...
        for x in img_ex:
            if x == file_extension:
                try:
                    ...
                    while not happens:
                        try:
                            os.rename(os.path.join(downloads, file), os.path.join(home + "\\Pictures\\Downloads", file + f" ({file_attempts})"))
                            happens = True
                        except FileExistsError:
                            file_attempts += 1
                break
        for x in vid_ex:
            if x == file_extension:
                try:
                    ...
                    while not happens:
                        try:
                            os.rename(os.path.join(downloads, file), os.path.join(home + "\\Videos\\Downloads", file + f" ({file_attempts})"))
                            happens = True
                        except FileExistsError:
                            file_attempts += 1
                break
        for x in doc_ex:
            if x == file_extension:
                try:
                    ...
                    while not happens:
                        try:
                            os.rename(os.path.join(downloads, file), os.path.join(home + "\\Documents\\Downloads", file + f" ({file_attempts})"))
                            happens = True
                        except FileExistsError:
                            file_attempts += 1
                break
        for x in music_ex:
            if x == file_extension:
                try:
                    ...
                    while not happens:
                        try:
                            os.rename(os.path.join(downloads, file), os.path.join(home + "\\Music\\Downloads", file + f" ({file_attempts})"))
                            happens = True
                        except FileExistsError:
                            file_attempts += 1
                break

    if (pics + docs + vids + mus) > 0:
        toast = ToastNotifier()
